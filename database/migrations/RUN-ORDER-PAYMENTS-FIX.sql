-- ============================================
-- FIX: Create order_payments table (run in Supabase SQL Editor)
-- ============================================
-- Use this if you see: "Could not find the table 'public.order_payments' in the schema cache"
-- Run the entire script in: Supabase Dashboard → SQL Editor → New query → Paste → Run

-- 1) Ensure trigger function exists (safe to run if already exists)
...
-- 2) Ensure is_admin() exists for RLS (safe to run if already exists)
